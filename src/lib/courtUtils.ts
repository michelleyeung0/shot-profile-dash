...
const COURT_RIGHT = SVG_WIDTH - MARGIN_X;
const COURT_TOP = MARGIN_Y;
const COURT_BOTTOM = SVG_HEIGHT - 20;
 ...
 * SVG half court is rotated 90 degrees counterclockwise from data coordinates, so:
 * - dataX (baseline to half court) maps to SVG vertical (y)
 * - dataY (sideline to sideline) maps to SVG horizontal (x)
 * @param dataX
 * @param dataY
 * @returns
 */
export function toSVGCoords(
  ...
  // dataX=0 → COURT_TOP, dataX=-47 → COURT_BOTTOM
  const svgY =
    COURT_TOP +
